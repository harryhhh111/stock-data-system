...
def parse_facts(root: Any) -> list[XbrlFact]:
    ...
    非审计目标 fact 的空值/占位符(如 '—')跳过;成本/收入类目标 fact
    解析失败必须抛错(永不静默跳过审计目标)。
    """
    facts: list[XbrlFact] = []
    for order, el in enumerate(root.iter(f"{{{IX_NS}}}nonFraction")):
        ...
        try:
            ...
            if _localname(name) in COST_LOCALNAMES | REVENUE_LOCALNAMES:
...
def parse_inline_xbrl(html_text: str) -> tuple[list[XbrlFact], dict[str, XbrlContext]]:
    ...
    return parse_facts(root), parse_contexts(root)
