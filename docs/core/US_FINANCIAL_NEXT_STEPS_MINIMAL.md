...
> 更新日期：2026-07-29
> 当前起点：消费者切换前 current-only 全市场对比已通过（`UNEXPLAINED=0`）；固定 10 只个股分析 canary 与 10 家官方 10-K 外部抽查通过；生产消费者尚未全量切换；ROIC 暂停。
...
> 当前决定：固定 canary 与外部抽查均已通过，进入全部美股当前个股分析切换；
> 筛选器和 PIT 回测不随本次切换。
...
固定样本已完成页面观察及官方 10-K 对照。代码提供
`US_FINANCIAL_VERSION_CURRENT=1` 作为全部美股当前个股分析开关；任一股票的新
版本装配异常时仍自动回退旧查询。关闭开关并重启进程即可整体回退。

本任务单只授权当前个股分析，不授权同时切换筛选器或 PIT 回测。
