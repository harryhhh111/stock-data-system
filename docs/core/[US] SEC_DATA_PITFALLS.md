...
- 不能笼统认定对下游无影响：ROE/ROIC 的权益口径、NCI 归属以及同日 annual/quarterly 版本选择都可能受影响。短期内某些现有指标未引用该列，不代表未来派生指标可以忽略；以财务指标前置数据治理方案为准。
...
- **单期指标**：毛利率/净利率在分子分母严格来自同一 duration 时可能部分抵消，但不能据此宣称无影响；ROE 的分子是期间流量、分母是时点权益，不会因“同一行”自动抵消。报告类型、期间和权益配对必须单独验证。
...
**处理方式**：在旧链路中曾作为已知限制；新治理要求纳入 TTM 质量标记和回归样本。具备可靠 standalone 数据时用四季度求和交叉验证，否则保留公式法并输出期间/质量信息。
