...
| US-7 | 中概股 CNY 数据被标为 USD（✅ 已修复） | SEC 同时提供 CNY 和 USD 版本，CNY 字典序靠前被 dedup 保留；transformer 硬编码 currency=USD | fetcher 层 `_KEEP_UNITS` 过滤只取 USD/shares 单位 |
...
| 中概股币种 | ~~JD/PDD/BIDU 的 CNY 值被标为 USD~~ | ~~FCF Yield 虚高 ~7x~~ | ✅ 已修复（fetcher 过滤只取 USD 单位，193 行数据已重新同步） |
