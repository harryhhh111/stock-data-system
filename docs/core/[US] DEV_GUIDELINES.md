...
| Standalone 列 | 42 个 `_standalone` 列存储单季度值 | CF 独立版覆盖率仅 31/504 | ✅ Schema+fetcher 完成，回填 504 只 |
| 交叉验证 | standalone 跨季度求和需知财年边界 | 误报率极高 | 🔄 暂用行级检查，等 DB 存 `frame` 字段 |
...
**数据覆盖（回填后）：** IS standalone 484/504 只，CF standalone 31/504 只。
验证函数 `check_standalone_cross_validation_us()` 当前做行级检查（负收入检测），跨季度求和需等 `frame` 字段存入 DB。
