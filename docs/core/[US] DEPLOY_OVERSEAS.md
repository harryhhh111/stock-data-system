...
- **挂载失效即"假归档"**：cosfs 掉线时写入会落到本机挂载点目录而非 COS。归档工具对
  `file:///lhcos-data` 强制检查 `mountpoint -q /lhcos-data` 和文件系统类型 `fuse.cosfs`；
  归档后仍应核对本机磁盘余量不因上传而额外减少（Phase E-0 实测判据：df 余量变化 ≈ 本地
  staging 目录大小）。
