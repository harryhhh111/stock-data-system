...
> 最后更新：2026-05-15（v1.5 — 开放问题全部对齐，方案就绪可实现）
...
**v1.5（2026-05-15）** 开放问题对齐：

1. **基准默认值**：US 默认 SPY 是行业标准（Bloomberg / Wind / 通达信），`--benchmark ''` 已支持禁用 → **采纳现方案**。
2. **CN 基准方案**：建议先跳过。`daily_quote` 里 A 股行情来源是腾讯接口，510300.SH 这类 ETF 行情需独立验证数据源覆盖。**独立任务，不阻塞 US 实现**。
3. **分红再投资**：unadjusted close 长期回测（5 年+）系统性低估基准约 1.5%/年，导致策略 alpha 虚高；短期（1-3 年）影响小。**v1 先用 unadjusted，跑通后对比 adjusted 差异**，差异 < 0.5%/年则保留，否则切换。
...
无 — 三个开放问题（基准默认值 / CN 基准 / 分红再投资）均已在 v1.5 评审中对齐，详见评审历史。**方案就绪，可以进入开发阶段。**

## 已解决的讨论点

- ~~基准默认值~~（v1.0 #1）→ v1.5 US 默认 SPY 是行业标准，`--benchmark ''` 已可禁用
- ~~CN 基准方案~~（v1.0 #2）→ v1.5 跳过，独立任务（需先验证 510300.SH 数据源覆盖），不阻塞 US 实现
- ~~分红再投资~~（v1.0 #3）→ v1.5 v1 先用 unadjusted，跑通后对比 adjusted 差异；差异 < 0.5%/年则保留
