...
> 创建时间：2026-06-15  
> 状态：US MVP 已上线并自动运行；引擎测试已补齐，复合策略实盘式验收待完善
> 前置阶段：复合策略前后端打通
...
2026-07-17 已新增 `tests/test_paper/`，测试不访问生产数据库：

- 引擎单元测试：账户现金恢复、注入连接、NAV/日收益/回撤、聚合持仓、费用和滑点、账户状态校验
- 连续多日工作流：首次建仓 → 非调仓日估值 → 跨月换仓 → 下跌估值 → 同日重复运行
- 幂等验证：重复运行不增加 NAV、交易或运行记录
- 完整测试结果：187 项通过

测试同时发现并修复：账户已有持仓时，数据库恢复流程没有把账户级剩余现金放回组合，可能低估非调仓日 NAV。普通策略现恢复到 `base`，复合策略恢复到 residual 子策略。
