...
### 6.2 引擎自动化验证（2026-07-17）

- 8 个核心单元测试
- 2 个工作流测试（连续四交易日 + 持仓账户现金恢复）
- 覆盖调仓、估值、换仓、日收益、回撤、费用/滑点和幂等
- 使用内存连接和 Mock 行情，不读写生产模拟盘表
- 项目完整测试：187 项通过

测试发现并修复了持仓账户的剩余现金恢复问题，避免非调仓日 NAV 漏计现金。
...
1. ✅ 运行时间：国内服务器北京时间 18:30；US 服务器北京时间 06:30
