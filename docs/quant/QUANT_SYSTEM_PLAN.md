...
- **普通因子回测**：验证单个选股预设在历史区间内的表现
- **复合策略回测**：支持商品信号、资金分配、多子策略独立调仓和汇总 NAV
- **模拟盘计划**：在复合策略前后端打通后，进入每日纸面组合跟踪

模拟盘仍然不自动下单；它用于观察真实时间流中的信号、持仓、净值和调仓建议。
...
│   │   ├── composite.py           # 复合策略引擎（多子策略资金分配 + 独立调仓 + 汇总）
