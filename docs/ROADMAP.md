...
> 最后更新：2026-06-15（复合策略落地 + 模拟盘前置计划）
...
- [ ] 复合策略前后端打通，`commodity_rotation` 可在 Web 端创建、运行和查看结果
- [ ] 策略运行结果可稳定序列化：持仓、现金、目标权重、成交建议、日频 NAV
- [ ] 明确模拟盘账户模型：初始资金、费用、滑点、调仓日、成交价规则
- [ ] 建立每日任务：行情更新后生成当日估值、信号状态和调仓建议
