...
### `run_initial_sync(days=30)` / `sync_financial_data(days=30)`

1. **sync_stock_list()**: 获取全部A股 + 港股列表 → `stock_info`
2. **sync_index_constituent()**: 获取沪深300/中证500/恒生成分 → `index_constituent`
3. **sync_financial_data(days)**: 遍历所有股票，逐个获取：
   - `fetch_financial_indicator()` → 写入 `financial_indicator`（含 PE/PB/ROE/市值等）
   - `fetch_and_save_cash_flow()` → 获取现金流量表，计算 FCF，**回写**到 `financial_indicator.fcf/fcf_yield`
